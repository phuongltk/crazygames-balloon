(() => {
  ...
  const saveHighscore = () => localStorage.setItem(STORAGE.highscore, String(highscore));
  const saveCoins = () => localStorage.setItem(STORAGE.coins, String(coins));
  const saveOwned = () => localStorage.setItem(STORAGE.owned, JSON.stringify(owned));
  ...
  function showScreen(el) {
    for (const s of [menuScreen, pauseScreen, gameOverScreen, shopScreen, adScreen]) {
  ...
  // ---------- Game state ----------
  const STATE = { MENU: 'menu', PLAYING: 'playing', PAUSED: 'paused', GAMEOVER: 'gameover', SHOP: 'shop', AD: 'ad' };
  ...
  shopBtn.addEventListener('click', () => { renderShop(); showScreen(shopScreen); });
  closeShopBtn.addEventListener('click', () => showScreen(menuScreen));
  pauseBtn.addEventListener('click', pauseGame);
  resumeBtn.addEventListener('click', resumeGame);
  quitBtn.addEventListener('click', () => { showScreen(null); quitToMenu(); });
